...
public class ParkingSpotController {
    ...
    public ResponseEntity<Page<ParkingSpotModel>> getAllParkingSpots(@PageableDefault(page = 0, size = 10, sort = "id", direction = Sort.Direction.ASC) Pageable pageable){
        return ResponseEntity.status(HttpStatus.OK).body(parkingSpotService.findAll(pageable)); // traz toda a listagem
    }

    @GetMapping("/{id}")
    public ResponseEntity<Object> getOneParkingSpot(@PathVariable(value = "id") UUID id){ //  ResponseEntity<Object> traz uma resposta caso caia em alguma verificação.
    ...
    public ResponseEntity<Object> updateParkingSpot(@PathVariable(value = "id") UUID id,
        ...
        var parkingSpotModel = new ParkingSpotModel();
        BeanUtils.copyProperties(parkingSpotDto, parkingSpotModel); // converte dto para model antes de salvar novamente no DB
